...
export const Stack = ({ children, className, gap = 2, align = 'start' }: _BaseProps & _SpacingProps) => {
    return (
        <div className={`flex flex-col ${gapStyles[gap]} ${alignStyles[align]} ${className}`}>
            {children}
        </div>
    )
};

export const Inline = ({ children, className, gap = 2, align = 'center' }: _BaseProps & _SpacingProps) => {
    return (
        <div className={`flex flex-row ${gapStyles[gap]} ${alignStyles[align]} ${className}`}>
            {children}
        </div>
    )
};
...
interface _TextProps {
    children: React.ReactNode;
    className?: string;
    variant: 'main-title' | 'main-subtitle' | 'main-body' | 'section-header' | 'section-title' | 'section-meta-text' | 'section-title-secondary' | 'section-body';
}
